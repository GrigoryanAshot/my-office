...
export default function TakhtPage() {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      ...
      setError(null);
      try {
        console.log('Fetching takht data...');
        const response = await fetch('/api/takht', {
          method: 'GET',
          headers: {
        ...
        if (!response.ok) {
          ...
          throw new Error(`Failed to fetch takht: ${response.status} ${errorText}`);
        }
        
        const data = await response.json();
        console.log('Takht data received:', data);
        ...
        console.error('Error loading takht:', error);
        setError(error instanceof Error ? error.message : 'Failed to load takht');
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className={styles.mainContainer} style={{ marginTop: '100px' }}>
        ...
        <div className={styles.grid}>
          {currentItems.map((item: FurnitureItem) => (
            <Link key={item.id} href={`/furniture/takht/${item.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
