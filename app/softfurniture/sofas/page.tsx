    ...
    const fetchItems = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(`Failed to fetch sofas: ${response.status} ${errorText}`);
        }
        
        const data = await response.json();
        console.log('Sofas data received:', data);
        ...
        setError(error instanceof Error ? error.message : 'Failed to load sofas');
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className={styles.mainContainer} style={{ marginTop: '100px' }}>
        ...
        <div className={styles.grid}>
          {currentItems.map((item: FurnitureItem) => (
            <Link key={item.id} href={`/furniture/sofas/${item.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
