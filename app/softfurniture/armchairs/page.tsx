...
export default function ArmchairsPage() {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      ...
      setError(null);
      try {
        console.log('Fetching armchairs data...');
        const response = await fetch('/api/armchairs', {
          method: 'GET',
          headers: {
        ...
        if (!response.ok) {
          ...
          throw new Error(`Failed to fetch armchairs: ${response.status} ${errorText}`);
        }
        
        const data = await response.json();
        console.log('Armchairs data received:', data);
        ...
        console.error('Error loading armchairs:', error);
        setError(error instanceof Error ? error.message : 'Failed to load armchairs');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <>
      ...
      <div className={styles.mainContainer} style={{ marginTop: '100px' }}>
        <h1 className={styles.description1}>Բազկաթոռներ</h1>
        ...
        <div className={styles.grid}>
          {currentItems.map((item: FurnitureItem) => (
            <Link key={item.id} href={`/furniture/armchairs/${item.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
