...
import WallDecorDetailClient from './WallDecorDetailClient';
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const redisKey = getRedisKeyForCategory('wall_decor');
...
export default async function WallDecorDetailPage({ params }: PageProps) {
  const { id } = await params;
  const redisKey = getRedisKeyForCategory('wall_decor');
  const categoryName = getCategoryDisplayName('wall_decor');
  ...
  return (
    <>
      <ProductSchema
        ...
        basePath="other"
      />
      <WallDecorDetailClient item={product} />
    </>
  );
} 