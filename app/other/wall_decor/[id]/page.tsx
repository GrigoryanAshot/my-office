...
import styles from './WallDecorDetail.module.css';
import DataLoading from '@/component/loading/DataLoading';

interface WallDecorItem {
  id: number;
  name: string;
  description: string;
  price: string;
...
export default function WallDecorDetailPage() {
  const params = useParams();
  const id = Array.isArray(params?.id) ? params.id[0] : params?.id;
  const router = useRouter();
  const [item, setItem] = useState<WallDecorItem | null>(null);
  ...
  useEffect(() => {
    const fetchItem = async () => {
      try {
        const response = await fetch(`/api/wall-decor/${id}`);
        if (!response.ok) {
