...
export default function MetalPage() {
  ...
  useEffect(() => {
    const loadCategories = async () => {
      ...
      const metalSubCategories = Object.entries(categories)
        .filter(([key, cat]) => key.startsWith('metal_') && key !== 'metal')
        .map(([key, cat]) => cat)
        .sort((a, b) => a.order - b.order);
      setSubCategories(metalSubCategories);
    };
    loadCategories();
  }, []);
  ...
  return (
    <>
      ...
      <div className={styles.categoryTitle}>Այլ</div>
