...
export default function AdminPanel() {
  ...
  // Load items from API when category is selected
  useEffect(() => {
  ...
  const handleSaveCategoryEdit = async () => {
    if (editingCategory && newCategory.name) {
      ...
      try {
        ...
        // Force a refresh of the navbar and main menu
  ...
  const handleMoveUp = async (key: string) => {
    ...
    const updatedCategories = { ...categories };
    const prevCategory = Object.values(categories).find(c => c.order === order - 1);
    
    if (prevCategory && prevCategory.order) {
      updatedCategories[key] = { ...category, order: order - 1 };
      updatedCategories[prevCategory.id] = { ...prevCategory, order: prevCategory.order + 1 };
  ...
  const handleMoveDown = async (key: string) => {
    ...
    const maxOrder = Math.max(...Object.values(categories)
      .map(c => c.order || 0));
    
    if (order >= maxOrder) return;

    const updatedCategories = { ...categories };
    const nextCategory = Object.values(categories).find(c => c.order === order + 1);
    
    if (nextCategory && nextCategory.order) {
      updatedCategories[key] = { ...category, order: order + 1 };
      updatedCategories[nextCategory.id] = { ...nextCategory, order: nextCategory.order - 1 };
  ...
  const handleMoveToFirst = async (key: string) => {
    const category = categories[key];
    const order = category.order;
    if (!order || order === 1) return;

    const updatedCategories = { ...categories };
    const categoriesToUpdate = Object.values(categories)
      .filter(c => c.order && c.order < order)
      .map(c => ({ ...c, order: (c.order || 0) + 1 }));
    ...
    updatedCategories[key] = { ...category, order: 1 };
  ...
  const handleMoveToLast = async (key: string) => {
    const category = categories[key];
    const order = category.order;
    if (!order) return;
    ...
    updatedCategories[key] = { ...category, order: maxOrder };
  ...
  const renderActionContent = () => {
    ...
    // Show edit/add form only when editing or adding
    ...
    // Otherwise, always show the admin grid
