...
const dataPath = path.join(process.cwd(), 'data', 'metal_metal_wall_decor_database.json');
...
if (!fs.existsSync(dataPath)) {
  try {
    ...
    console.error('Error creating metal_metal_wall_decor_database.json:', error);
  }
}
...
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: 'Failed to read wall decor data', details: error instanceof Error ? error.message : String(error) }, { status: 500 });
  }
}
