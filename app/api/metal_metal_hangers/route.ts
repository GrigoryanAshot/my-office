...
if (!fs.existsSync(dataPath)) {
  try {
    ...
    console.error('Error creating metal_metal_hangers_database.json:', error);
  }
}
...
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: 'Failed to read hangers data', details: error instanceof Error ? error.message : String(error) }, { status: 500 });
  }
}
