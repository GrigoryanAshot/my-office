...
// Use a single database file for tables
...
// Initialize tables_database.json if it doesn't exist
if (!fs.existsSync(tablesDataPath)) {
  try {
    const initialData = { items: [], types: [] };
    fs.writeFileSync(tablesDataPath, JSON.stringify(initialData, null, 2), 'utf8');
  } catch (error) {
    console.error('Error creating tables_database.json:', error);
  }
}

export async function GET() {
  try {
    console.log('GET /api/tables - Reading data from:', tablesDataPath);
    ...
    return NextResponse.json({ error: 'Failed to read tables data', details: error instanceof Error ? error.message : String(error) }, { status: 500 });
  }
}

export async function POST(request: Request) {
  try {
    console.log('POST /api/tables - Received request');
    
    // Parse the request body
    const data = await request.json();
    console.log('POST /api/tables - Request data:', JSON.stringify(data, null, 2));
    ...
    let currentData;
    try {
      const fileContent = fs.readFileSync(tablesDataPath, 'utf8');
    ...
    try {
      console.log('Writing data to file:', tablesDataPath);
      fs.writeFileSync(tablesDataPath, JSON.stringify(updatedData, null, 2), 'utf8');
      console.log('Data written successfully');
      
      // Verify the data was written correctly
      const writtenData = fs.readFileSync(tablesDataPath, 'utf8');
      console.log('Verification - Data read back:', writtenData);
    } catch (error) {
      console.error('Error writing to tables_database.json:', error);
