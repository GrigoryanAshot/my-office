    ...
    const fetchItems = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(`Failed to fetch stands: ${response.status} ${errorText}`);
        }
        
        const data = await response.json();
        console.log('Stands data received:', data);
        ...
        setError(error instanceof Error ? error.message : 'Failed to load stands');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <>
      ...
      <div className={styles.mainContainer} style={{ marginTop: '100px' }}>
        <h1 className={styles.description1}>Տակդիրներ</h1>
        ...
        <div className={styles.grid}>
          {currentItems.map((item: FurnitureItem) => (
            <Link key={item.id} href={`/furniture/stands/${item.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
