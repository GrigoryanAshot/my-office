...
export default function ShelvingPage() {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      ...
      setError(null);
      try {
        console.log('Fetching shelving data...');
        const response = await fetch('/api/shelving', {
          method: 'GET',
          headers: {
        ...
        if (!response.ok) {
          ...
          throw new Error(`Failed to fetch shelving: ${response.status} ${errorText}`);
        }
        
        const data = await response.json();
        console.log('Shelving data received:', data);
        ...
        console.error('Error loading shelving:', error);
        setError(error instanceof Error ? error.message : 'Failed to load shelving');
      } finally {
        setLoading(false);
      }
    };
