...
export default function ChestsPage() {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      ...
      setError(null);
      try {
        console.log('Fetching chests data...');
        ...
        if (!response.ok) {
          ...
          throw new Error(`Failed to fetch chests: ${response.status} ${errorText}`);
        }
        
        const data = await response.json();
        console.log('Chests data received:', data);
        ...
        setError(error instanceof Error ? error.message : 'Failed to load chests');
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className={styles.mainContainer} style={{ marginTop: '100px' }}>
        ...
        <div className={styles.grid}>
          {currentItems.map((item: FurnitureItem) => (
            <Link key={item.id} href={`/furniture/chests/${item.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
