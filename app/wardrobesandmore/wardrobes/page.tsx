...
export default function WardrobesPage() {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      ...
      setError(null);
      try {
        console.log('Fetching wardrobes data...');
        ...
        if (!response.ok) {
          ...
          throw new Error(`Failed to fetch wardrobes: ${response.status} ${errorText}`);
        }
        
        const data = await response.json();
        console.log('Wardrobes data received:', data);
        ...
        setError(error instanceof Error ? error.message : 'Failed to load wardrobes');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <>
      ...
      <div className={styles.mainContainer} style={{ marginTop: '100px' }}>
        <h1 className={styles.description1}>Պահարաններ</h1>
