...
import styles from './ChestDetail.module.css';
...
export default function ChestDetailPage() {
  const params = useParams();
  const id = Array.isArray(params?.id) ? params.id[0] : params?.id;
  const router = useRouter();
  const [item, setItem] = useState<ChestItem | null>(null);
  ...
  useEffect(() => {
    const fetchItem = async () => {
      try {
        const response = await fetch(`/api/chests?t=${Date.now()}&r=${Math.random()}`, {
          cache: 'no-store',
          headers: {
        ...
        if (!response.ok) {
          throw new Error('Failed to fetch chests data');
        }
