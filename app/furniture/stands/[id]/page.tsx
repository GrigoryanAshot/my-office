...
import styles from './StandDetail.module.css';
...
export default function StandDetailPage() {
  ...
  const [item, setItem] = useState<StandItem | null>(null);
  ...
  useEffect(() => {
    const fetchItem = async () => {
      try {
        const response = await fetch(`/api/stands?t=${Date.now()}&r=${Math.random()}`, {
          cache: 'no-store',
          headers: {
